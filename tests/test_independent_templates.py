...
def create_template_test(
    ...
    train_config,
    eval_config,
    data_config="demo_tabular.csv",
    data_block="ingest.csv",
):
...
"""
            ...
            print("  📦 Installing framework locally...")
            install_cmd = [sys.executable, "-m", "pip", "install", "-e", ".", "--quiet"]
            install_result = subprocess.run(
                install_cmd, cwd=tmp_path, capture_output=True, text=True
            )
            ...
            result = subprocess.run(
                run_cmd, cwd=tmp_path, capture_output=True, text=True, timeout=300
            )
            ...
            if (
                ...
                for line in lines:
                    if (
                        "accuracy:" in line.lower()
                        or "f1_score:" in line.lower()
                        or "mse:" in line.lower()
                        or "mae:" in line.lower()
                    ):
                        print(f"    📊 {line.strip()}")
...
def main():
    ...
    print(
        f"📊 Independent Template Test Results: {passed}/{total} tests passed ({percentage:.1f}%)"
    )
