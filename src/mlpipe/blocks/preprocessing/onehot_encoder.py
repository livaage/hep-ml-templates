...
class OneHotEncoderBlock(Preprocessor):
    ...
    def build(self, config: Dict[str, Any]) -> None:
        ...
        if self.config['verbose']:
            print(f"🎯 One-Hot Encoder Configuration:")
    ...
    def _detect_categorical_columns(self, X: Union[np.ndarray, pd.DataFrame]) -> List[Union[int, str]]:
    ...
    def fit(self, X: Union[np.ndarray, pd.DataFrame], y: Optional[np.ndarray] = None) -> 'OneHotEncoderBlock':
        ...
        if self.config['verbose']:
            print(f"✅ One-hot encoder fitted:")
            if isinstance(X, pd.DataFrame):
    ...
    def transform(self, X: Union[np.ndarray, pd.DataFrame]) -> Union[np.ndarray, pd.DataFrame]:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            if self.config['sparse_output']:
                ...
                numerical_columns = [i for i in range(X.shape[1]) if i not in self.categorical_columns]
                if numerical_columns:
        ...
        if hasattr(self.encoder, 'get_feature_names_out'):
            ...
            if self.feature_names is not None:
                ...
                if isinstance(self.encoder, ColumnTransformer):
                    n_features = self.encoder.transform(np.zeros((1, len(input_features or [])))).shape[1]
                else:
                    n_features = self.encoder.transform(np.zeros((1, len(self.categorical_columns)))).shape[1]
