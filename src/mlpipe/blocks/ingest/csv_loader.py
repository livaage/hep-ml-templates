...
class UniversalCSVLoader(DataIngestor):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.drop_missing_threshold = config.get('drop_missing_threshold', 0.5)  # Drop cols with >50% missing
        self.categorical_columns = config.get('categorical_columns', [])
        self.numerical_columns = config.get('numerical_columns', [])
        
        # Target processing
        self.target_type = config.get('target_type', 'auto')  # 'binary', 'multiclass', 'regression', 'auto'
        self.positive_class_labels = config.get('positive_class_labels', [1, '1', 'yes', 'true', 'signal'])
        ...
        try:
            ...
            if not self.has_header:
                ...
                if len(df.columns) == len(self.column_names):
                    ...
                    warnings.warn(f"Column name count mismatch: got {len(self.column_names)}, expected {len(df.columns)}")
                return df
    ...
    def _preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing_stats.sum() > 0:
            ...
            high_missing_cols = missing_stats[missing_stats > len(df) * self.drop_missing_threshold].index
            if len(high_missing_cols) > 0:
                df = df.drop(columns=high_missing_cols)
                if self.verbose:
                    print(f"   Dropped {len(high_missing_cols)} columns with >{self.drop_missing_threshold*100}% missing")
    ...
    def _process_target(self, y: pd.Series) -> Tuple[pd.Series, Dict[str, Any]]:
        ...
        if target_info['task_type'] == 'binary':
            ...
            if y.dtype == 'object':
                ...
                le = LabelEncoder()
                y_processed = pd.Series(le.fit_transform(y.dropna()), index=y.dropna().index, name=y.name)
