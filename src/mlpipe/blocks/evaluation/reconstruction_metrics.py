...
class ReconstructionEvaluator(Evaluator):
    ...
    def build(self, config: Dict[str, Any]) -> None:
        ...
        default_config = {
            'metrics': ['mse', 'mae', 'rmse', 'snr'],  # Available: 'mse', 'mae', 'rmse', 'snr', 'ssim'
        ...
        if 'ssim' in self.config['metrics'] and not SSIM_AVAILABLE:
            print("⚠️  Warning: SSIM metric requires scikit-image. Install with: pip install scikit-image")
            self.config['metrics'] = [m for m in self.config['metrics'] if m != 'ssim']
            
        if self.config['verbose']:
            print(f"🔍 Reconstruction Evaluator Configuration:")
    ...
    def _compute_mse(self, original: np.ndarray, reconstructed: np.ndarray) -> Union[float, np.ndarray]:
        """Compute Mean Squared Error."""
        error = (original - reconstructed) ** 2
        if self.config['per_sample']:
            return np.mean(error, axis=tuple(range(1, error.ndim)))  # Mean over all dims except batch
        else:
            return np.mean(error)
            
    def _compute_mae(self, original: np.ndarray, reconstructed: np.ndarray) -> Union[float, np.ndarray]:
    ...
    def _compute_rmse(self, original: np.ndarray, reconstructed: np.ndarray) -> Union[float, np.ndarray]:
        """Compute Root Mean Squared Error."""
        mse = self._compute_mse(original, reconstructed)
        return np.sqrt(mse)
        
    def _compute_snr(self, original: np.ndarray, reconstructed: np.ndarray) -> Union[float, np.ndarray]:
        """Compute Signal-to-Noise Ratio in dB."""
        signal_power = np.mean(original ** 2, axis=tuple(range(1, original.ndim))) if self.config['per_sample'] else np.mean(original ** 2)
        noise_power = np.mean((original - reconstructed) ** 2, axis=tuple(range(1, original.ndim))) if self.config['per_sample'] else np.mean((original - reconstructed) ** 2)
    ...
    def _compute_ssim(self, original: np.ndarray, reconstructed: np.ndarray) -> Union[float, np.ndarray]:
        ...
        if self.config['per_sample']:
            ...
            for i in range(original_2d.shape[0]):
                ...
                if orig_img.max() > 1.0 or orig_img.min() < 0.0:
                    orig_img = (orig_img - orig_img.min()) / (orig_img.max() - orig_img.min() + self.config['epsilon'])
                    recon_img = (recon_img - recon_img.min()) / (recon_img.max() - recon_img.min() + self.config['epsilon'])
    ...
    def _plot_reconstructions(self, original: np.ndarray, reconstructed: np.ndarray, 
    ...
    def _save_samples(self, original: np.ndarray, reconstructed: np.ndarray, 
                     output_dir: Path) -> None:
        ...
        np.save(output_dir / 'reconstructed_samples.npy', reconstructed[:self.config['plot_samples']])
    ...
    def evaluate(self, original: np.ndarray, reconstructed: np.ndarray) -> Dict[str, Any]:
        ...
        if self.config['verbose']:
            print(f"✅ Reconstruction evaluation complete:")
